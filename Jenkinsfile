        ...
        stage('7 - DAST (OWASP ZAP)') {
            steps {
                ...
                bat '''
                    ...
                    docker run --rm --add-host=host.docker.internal:host-gateway -v "%CD%\\zap-report:/zap/wrk" -v "%CD%\\zap.conf:/zap/wrk/zap.conf" ghcr.io/zaproxy/zaproxy:stable zap-baseline.py -t http://host.docker.internal:9090 -r zap-pipeline.html -c /zap/wrk/zap.conf
                '''
            }
        }
    }
